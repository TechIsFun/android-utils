 ...
 */
public class RangeSeekBarDiscrete<T extends Number> extends RangeSeekBar<T> {

    private double normalizedStepSize;

    @SuppressWarnings("unchecked")
    public RangeSeekBarDiscrete(T absoluteMinValue, T absoluteMaxValue,
            T stepSize, Context context) throws IllegalArgumentException {
        super(absoluteMinValue, absoluteMaxValue, context);

        this.normalizedStepSize = valueToNormalized((T) numberType
                .toNumber(absoluteMinValue.doubleValue()
    ...
    private double findClosestNormalizedStep(double value) {
        ...
        double stepAbove = Math.min(normalizedStepSize * (numbStepsBelow + 1),
                1.0d);
